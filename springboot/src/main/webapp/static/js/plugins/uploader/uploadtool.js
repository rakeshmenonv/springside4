...
function startUploadOneData(imgname){
    ...
    html += '<div class="imgbox"><img src="../static/uploader/images/loading.gif" onload="photoin(this,60,60)" /></div>';
...
function uploadData(txt,id){    
    txts = recheak(txt);
    $("#res_"+id).children(".imgbox").children("img").attr("src",txt);  
...
function recheak(url){
    ...
    if("mp3|wma".indexOf(result)>=0){
        return "/admin/images/audio.gif";
    }
    else if("flv|swf".indexOf(result)>=0){
        return "/admin/images/video.gif";
    }
    else{
        return url;
    }
